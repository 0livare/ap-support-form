...
const schema = z.object({
  ...
  formsFile: z.string().optional(),
  digiEnvelope: z.string().optional(),
...
function SimpleForm() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-linear-to-br from-purple-100 to-blue-100 p-4">
      <div className="w-full max-w-2xl p-8 rounded-xl backdrop-blur-md bg-white shadow-xl">
        <form
          onSubmit={(e) => {
            e.preventDefault()
            e.stopPropagation()
              ...
              <field.FileUpload label="Are there any screenshots that would be useful in addition to your video?" />
                  ...
                  {app === 'forms' && (
                    <form.AppField name="formsFile">
                      {(field) => (
                        <field.TextField label="Do you have a link to the Forms File in question?" />
                      )}
                    </form.AppField>
                  )}
                  {app === 'digisign' && (
                    <form.AppField name="formsFile">
                      {(field) => (
                        <field.TextField label="Do you have a link to the Digisign envelope in question?" />
                      )}
                    </form.AppField>
                  )}
